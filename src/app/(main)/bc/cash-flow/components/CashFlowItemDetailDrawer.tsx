...
import { useBCCashFlowItemDetail } from '../../hooks/useBCCashFlowItemDetail'
...
interface CashFlowItemDetailDrawerProps {
  open: boolean
  onClose: () => void
  connectionId: string
...
export function CashFlowItemDetailDrawer({
  open,
  onClose,
  connectionId,
  itemType,
  ...
  const chartSeries = useMemo(() => [{ key: 'amount', name: 'Amount', color: '#f59e0b' }], [])
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <h3 className={cn('text-lg font-semibold', isLight ? 'text-stone-900' : 'text-white')}>
              {itemName || label || 'Cash Flow Item Detail'}
            </h3>
            <p
              className={cn(
          ...
          <>
            ...
            {accountBreakdown.length > 0 && (
              <div>
                ...
                >
                  Account Breakdown ({accountBreakdown.length} accounts)
                </h4>
                <div
                  className={cn(
                ...
                >
                  <div
                    className={cn(
                      'grid grid-cols-[1fr_80px_80px_90px] gap-2 px-3 py-2 text-[10px] uppercase tracking-wider font-medium border-b border-[var(--theme-card-border)]',
                      isLight
                  ...
                  <div className="max-h-[200px] overflow-y-auto">
                    {accountBreakdown.map((acc, i) => (
                      <div
                        key={acc.number}
                        className={cn(
                          'grid grid-cols-[1fr_80px_80px_90px] gap-2 px-3 py-1.5 text-[12px] border-b last:border-b-0',
                          i % 2 === 0
                            ? isLight
                              ? 'bg-white'
                              : 'bg-transparent'
                      ...
                      >
                        ...
                        >
                          {acc.debit > 0 ? formatStatementAmount(acc.debit, currency) : '—'}
                        </span>
                        <span
                          className={cn(
                            'text-right font-mono tabular-nums text-[11px]',
                        ...
                        >
                          {acc.credit > 0 ? formatStatementAmount(acc.credit, currency) : '—'}
                        </span>
                        <span
                          className={cn(
                        ...
                        >
                          {formatStatementAmount(Math.abs(acc.net), currency)}
                        </span>
                      </div>
                    ))}
                  </div>
                ...
                return (
                  <div>
                    ...
                    >
                      <div
                        className={cn(
                          'grid grid-cols-[80px_1fr_70px_70px] gap-2 px-3 py-2 text-[10px] uppercase tracking-wider font-medium border-b border-[var(--theme-card-border)]',
                          isLight
                            ? 'bg-[var(--theme-bg)] text-stone-500'
                            : 'bg-white/[0.02] text-stone-500'
                        )}
                      ...
                      <div>
                        {pageRows.map((tx, i) => (
                          <div
                            ...
                            className={cn(
                              'grid grid-cols-[80px_1fr_70px_70px] gap-2 px-3 py-1.5 text-[12px] border-b last:border-b-0',
                              i % 2 === 0
                                ? isLight
                                  ? 'bg-white'
                                  : 'bg-transparent'
                          ...
                          >
                            ...
                            <span
                              ...
                              title={`${tx.accountNumber}: ${tx.description || tx.documentNumber}`}
                            >
            ...
            {recentTransactions.length === 0 && accountBreakdown.length === 0 && (
              <div
                className={cn(
              ...
              >
                No activity found for this item in the selected period
              </div>
            )}
          </>
        )}
