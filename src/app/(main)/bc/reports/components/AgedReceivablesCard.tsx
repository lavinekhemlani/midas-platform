...
import type { AgedReceivablesSummary } from '@/app/(main)/bc-warehouse/reports/hooks/useWarehouseData'

interface AgedReceivablesCardProps {
  summary: AgedReceivablesSummary | null
  isLoading: boolean
  currency?: string
  tooltipProps?: Omit<InfoTooltipProps, 'className'>
}

export function AgedReceivablesCard({
  summary,
  isLoading,
  currency = 'USD',
  tooltipProps,
  ...
  if (!summary) {
    return (
      <div>
        ...
        <p className={cn('text-sm py-4', styles.textMuted)}>No receivables data</p>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <span className="text-[28px] font-mono font-semibold tabular-nums block mb-4 text-amber-500">
      ...
      <div className={cn('text-xs pt-2 mt-2 border-t', styles.border, styles.textMuted)}>
        {summary.customer_count} customers with outstanding balances
      </div>
    </div>
  )
}
