...
import { BCGLAccountDetailView } from './views/BCGLAccountDetailView'
import { useBCConnection } from '@/hooks/useBCConnection'

export default function GLAccountDetailPage() {
  const params = useParams()
  const accountNumber = decodeURIComponent(params.accountNumber as string)
  ...
  return (
    <div className="@container space-y-4 max-w-[1800px] mx-auto">
      ...
      >
        <BCGLAccountDetailView connectionId={activeConnection.id} accountNumber={accountNumber} />
      </Suspense>
    </div>
  )
}
