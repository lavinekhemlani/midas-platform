...
interface CustomerConcentrationCardProps {
...
export function CustomerConcentrationCard({
  ...
  isLoading,
  currency = 'USD',
}: CustomerConcentrationCardProps) {
  ...
  const topCustomerPct = metrics?.topCustomerPercentage ?? 0
              ...
              <div className={cn('w-full h-2.5 rounded-full overflow-hidden flex', styles.barBg)}>
                {data.slice(0, 10).map((customer, i) => (
                  <div
                    ...
                    title={`${customer.name}: ${customer.percentage.toFixed(1)}%`}
                  />
                ))}
                {top10Pct < 100 && (
                  <div
