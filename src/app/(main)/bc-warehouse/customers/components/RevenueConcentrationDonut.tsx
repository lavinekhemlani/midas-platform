...
export function RevenueConcentrationDonut({
  data,
  metrics,
  isLoading,
  currency = 'USD',
}: RevenueConcentrationDonutProps) {
  ...
  const totalRevenue = useMemo(() => {
    return data.reduce((sum, v) => sum + v.sales_lcy, 0)
  }, [data])

  if (isLoading) return <Skeleton isLight={isLight} />

  if (!data.length) {
    return <div className={cn('py-6 text-center text-sm', styles.textMuted)}>No revenue data</div>
  }
  ...
  return (
    <div className="flex items-center gap-6">
          ...
          {hoveredSegment ? (
            <>
              ...
              <span
                ...
                title={hoveredSegment.customer.name}
              >
                {hoveredSegment.customer.name}
              </span>
            </>
          ) : (
            <>
      ...
      <div className="flex-1 min-w-0">
        ...
        {/* Top customers list - matching inventory list styling */}
        <div
          ...
          {segments.map((segment, i) => (
            ...
            >
              ...
              <span className={cn('font-mono tabular-nums font-semibold text-[11px]', styles.text)}>
                {formatCompactCurrency(segment.customer.sales_lcy, currency)}
              </span>
            </div>
          ))}
        </div>
