...
export function PaymentTermsCard({ data, isLoading, currency = 'USD' }: PaymentTermsCardProps) {
  const { theme } = useTheme()
  const isLight = theme === 'light'

  const totalCustomers = data.reduce((sum, p) => sum + p.customer_count, 0)
  const totalSales = data.reduce((sum, p) => sum + p.total_sales, 0)
  const maxSales = data.length > 0 ? Math.max(...data.map((p) => p.total_sales)) : 0
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {data.slice(0, 6).map((terms, i) => {
                const salesPercent = totalSales > 0 ? (terms.total_sales / totalSales) * 100 : 0
                const barWidth =
                  maxSales > 0 ? Math.max(8, (terms.total_sales / maxSales) * 100) : 0
                  ...
                  >
                    <div className="flex items-center justify-between gap-2 mb-1.5">
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        >
                          {formatCompactCurrency(terms.total_sales, currency)}
                        </span>
                      </div>
                    </div>
