...
import type { CreditRiskSummary } from '../hooks/useCustomerInsights'
...
interface CustomerHealthDashboardProps {
  ...
  riskSummary: CreditRiskSummary | null
  isLoading: boolean
  currency?: string
}
...
export function CustomerHealthDashboard({
  ...
  const healthScore = useMemo(() => {
    ...
    // - High risk customer ratio (lower is better)
    const utilizationScore = Math.max(0, 100 - (riskSummary.overall_utilization || 0))
    const overdueRatio =
      overview.total_ar > 0 ? (overview.total_overdue / overview.total_ar) * 100 : 0
    const overdueScore = Math.max(0, 100 - overdueRatio)
    const riskRatio =
      overview.total_customers > 0
        ? (riskSummary.high_risk_customers / overview.total_customers) * 100
        : 0
    const riskScore = Math.max(0, 100 - riskRatio * 2)

    return utilizationScore * 0.3 + overdueScore * 0.4 + riskScore * 0.3
  }, [overview, riskSummary])

  const overduePercentage = useMemo(() => {
    if (!overview || overview.total_ar === 0) return 0
    return (overview.total_overdue / overview.total_ar) * 100
  }, [overview])
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={cn('text-sm', isLight ? 'text-stone-500' : 'text-stone-400')}>
                Real-time receivables & risk analysis
              </p>
            </div>
            <div
              className={cn(
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <MetricTile
              ...
              subvalue={`${overview.customers_with_balance} with balance`}
              color="#6366f1"
              isLight={isLight}
            />
            <MetricTile
              icon={DollarSign}
              label="Total AR"
              value={formatCompactCurrency(overview.total_ar, currency)}
            ...
            <MetricTile
              ...
              value={formatCompactCurrency(overview.total_credit_limit, currency)}
              subvalue={`${riskSummary?.overall_utilization?.toFixed(0) || 0}% utilized`}
              color="#f59e0b"
              isLight={isLight}
            />
            <MetricTile
              ...
              value={(riskSummary?.high_risk_customers || 0).toString()}
              subvalue={`${riskSummary?.customers_overdue || 0} overdue`}
              trend={(riskSummary?.high_risk_customers || 0) > 5 ? 'down' : 'neutral'}
              color="#ef4444"
              isLight={isLight}
            />
          </div>
          ...
          >
            ...
            />
            <CircularGauge
              value={riskSummary?.overall_utilization || 0}
              color={
                riskSummary?.overall_utilization && riskSummary.overall_utilization > 80
                  ? '#ef4444'
                  : riskSummary?.overall_utilization && riskSummary.overall_utilization > 60
                    ? '#f59e0b'
                    : '#10b981'
              }
              label="Credit Utilization"
              sublabel="Limit usage"
              isLight={isLight}
            />
            <CircularGauge
          ...
          >
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                >
                  {formatCompactCurrency(overview.total_profit, currency)}
                </span>
              </div>
              <div className={cn('w-px h-8', isLight ? 'bg-stone-200' : 'bg-white/10')} />
              <div>
                ...
                >
                  {overview.blocked_customers} customers
                </span>
              </div>
            </div>
