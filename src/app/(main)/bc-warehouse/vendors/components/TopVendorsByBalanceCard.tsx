...
interface TopVendorsByBalanceCardProps {
  data: VendorBalance[]
  isLoading: boolean
  currency?: string
}
...
export function TopVendorsByBalanceCard({
  data,
  isLoading,
  currency = 'USD',
}: TopVendorsByBalanceCardProps) {
  const { theme } = useTheme()
  const isLight = theme === 'light'

  const maxBalance = data.length > 0 ? Math.max(...data.map((v) => v.balance_lcy)) : 0
  const totalBalance = data.reduce((sum, v) => sum + v.balance_lcy, 0)
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {data.slice(0, 8).map((vendor) => {
                const barWidth =
                  maxBalance > 0 ? Math.max(8, (vendor.balance_lcy / maxBalance) * 100) : 0
                const hasOverdue = vendor.balance_due_lcy > 0
                const overduePercent =
                  vendor.balance_lcy > 0 ? (vendor.balance_due_lcy / vendor.balance_lcy) * 100 : 0
