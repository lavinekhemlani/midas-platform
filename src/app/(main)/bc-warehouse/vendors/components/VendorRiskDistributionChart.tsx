...
interface VendorRiskDistributionChartProps {
...
const riskConfig = {
  ...
  high: {
    ...
    color: '#f97316',
    lightColor: '#ea580c',
    description: 'High overdue ratio or amount',
  },
  medium: {
    label: 'Medium',
    color: '#f59e0b',
    lightColor: '#d97706',
    description: 'Moderate overdue',
  },
  low: {
    label: 'Low',
    color: '#10b981',
    lightColor: '#059669',
    description: 'Healthy payment status',
  },
}
...
export function VendorRiskDistributionChart({
  data,
  summary,
  isLoading,
  currency = 'USD',
}: VendorRiskDistributionChartProps) {
  ...
  const distribution = useMemo(() => {
    ...
    data.forEach((vendor) => {
      counts[vendor.risk_score]++
      balances[vendor.risk_score] += vendor.balance_lcy
    })
    ...
    <div className="flex items-center gap-6">
      ...
      <div className="relative flex-shrink-0" style={{ width: size, height: size }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className={cn('text-2xl font-bold tabular-nums', styles.text)}>{totalVendors}</span>
