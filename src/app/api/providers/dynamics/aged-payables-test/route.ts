...
/**
 * Aged Payables Diagnostic — Probe multiple endpoint variations
 *
 * Tests many entity name / approach combinations to find what works:
 *  1. agedAccountsPayable (current code — singular)
 *  2. agedAccountsPayables (BC docs — plural)
 *  3. vendors with $select including balance, currencyCode
 *  4. purchaseInvoices filtered to Open status
 *  5. generalLedgerEntries for payables
 *
...
export const GET = withAuth(async (request: NextRequest, { organizationId }) => {
  try {
    ...
    const probes = await Promise.all([
      ...
      probe(
        'aged-ap-singular',
        'agedAccountsPayable (singular)',
        'Current code entity name — may be wrong per BC docs',
        () => client.query('agedAccountsPayable').then((r) => r.value)
      ),

      // Probe 2: BC docs entity name (plural)
      probe(
        'aged-ap-plural',
        'agedAccountsPayables (plural)',
        'Official BC API v2.0 entity name per Microsoft docs',
        () => client.query('agedAccountsPayables').then((r) => r.value)
      ),

      // Probe 3: Try with $top=5 to limit response
      probe(
        'aged-ap-plural-top5',
        'agedAccountsPayables?$top=5',
        'Plural name with $top=5 — tests OData params work',
        () => client.query('agedAccountsPayables', { $top: 5 }).then((r) => r.value)
      ),

      // Probe 4: Vendors with all financial fields
      probe(
        'vendors-full',
        'vendors (full financial fields)',
        'Vendor list with balance, currencyCode, taxRegistrationNumber',
        () =>
          client.listVendors({
            $select: 'id,number,displayName,balance,currencyCode',
            $orderby: 'displayName',
          })
      ),

      // Probe 5: Open purchase invoices (alternative aging source)
      probe(
        'open-purchase-invoices',
        "purchaseInvoices (status eq 'Open')",
      ...
      // Probe 6: All purchase invoices (small sample)
      probe(
        'purchase-invoices-sample',
        'purchaseInvoices ($top=5, all statuses)',
      ...
      // Probe 8: Try vendorLedgerEntries (may be beta / unavailable)
      probe(
        'vendor-ledger-entries',
        'vendorLedgerEntries ($top=5)',
        'Finance Reports API (Beta) — detailed vendor transactions with remaining amounts',
        () => client.query('vendorLedgerEntries', { $top: 5 }).then((r) => r.value)
      ),

      // Probe 9: Try detailedVendorLedgerEntries
      probe(
        'detailed-vendor-ledger',
        'detailedVendorLedgerEntries ($top=5)',
        'Finance Reports API (Beta) — granular applications and adjustments',
        () => client.query('detailedVendorLedgerEntries', { $top: 5 }).then((r) => r.value)
      ),

      // Probe 10: GL entries sample
      probe(
        'gl-entries-sample',
    ...
    logger.info('Aged AP diagnostic probes complete', {
      organizationId,
    ...
    logger.error('Aged AP diagnostic failed', { organizationId, error })
    return NextResponse.json(
      {
        error: 'Aged payables diagnostic failed',
